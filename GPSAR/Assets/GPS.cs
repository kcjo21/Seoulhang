...
public class GPS : MonoBehaviour {
    ...
    private IEnumerator StartLocationService()
    {
        ...
        if (locale == "ko")
        {
            _dbcm.CommandText = "SELECT question_name,question_code FROM questions WHERE (abs(x_coordinate-" + latitude + "))*(abs(x_coordinate-" + latitude + "))+(abs(y_coordinate-" + longitude + "))*(abs(y_coordinate-" + longitude + "))<=(0.002)*(0.002)"; //현재위치에서 500m안에 있는 역
        }
        else if (locale == "en")
        {
            _dbcm.CommandText = "SELECT question_name,question_code FROM questions WHERE (abs(x_coordinate-" + latitude + "))*(abs(x_coordinate-" + latitude + "))+(abs(y_coordinate-" + longitude + "))*(abs(y_coordinate-" + longitude + "))<=(0.002)*(0.002)"; //현재위치에서 500m안에 있는 역
        ...
        while (_dbr.Read())
        {
            ...
            region_n = _dbr.GetString(0);       
            question_c = _dbr.GetInt32(1);
            if (!quizlist.Contains(question_c.ToString()))
            {
                region_name = region_n;
                region_code = question_c.ToString();
            }
       ...
       /* region_code = region_c;
        if (region_code == 26 && locale == "ko")
            region_name = region_n + "님의 퀴즈";
        else if (region_code == 26 && locale == "en")
            region_name = region_n + "'s Quiz";
        Debug.Log(region_n);*/
